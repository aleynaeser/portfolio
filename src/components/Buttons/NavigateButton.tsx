...
export default function NavigateButton({ text, href, className, Icon }: INavigateButton) {
  return (
    <Link
      href={href}
      className={`group relative flex w-fit items-center gap-2 overflow-hidden rounded-3xl border border-transparent bg-primary px-5 py-2 text-xs text-white ${className}`}
    >
